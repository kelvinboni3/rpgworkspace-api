...
public sealed class AnthropicNoteStructuringGateway : INoteStructuringGateway
{
    ...
    private static string BuildSystemPrompt() => """
        ...
        8. Antes de criar um bloco novo, confira a lista de blocos já existentes deste personagem. Se a anotação trouxer informação NOVA sobre o mesmo assunto/NPC/item/lugar de um bloco que já existe, prefira ATUALIZAR aquele bloco em vez de duplicar: preencha "targetBlockId" com o id dele e, em "content" (ou no "payloadJson", se for Card/Table), coloque APENAS a informação NOVA a acrescentar — NÃO repita o conteúdo que já está no bloco. O aplicativo anexa automaticamente o que você mandar ao conteúdo que já existe, então reescrever o texto antigo só causaria duplicação, e você nunca precisa reproduzir o que já está lá (assim também nunca há risco de apagar o antigo). Para Card/Table, envie só as linhas novas; só reenvie uma linha existente se o valor dela mudou DE FATO (aí mande só essa linha corrigida). Essa regra é por ENTIDADE: se a anotação cita 3 pessoas e só 1 delas já tem bloco, atualize o bloco dessa 1 e crie blocos novos pras outras 2 — não use o bloco existente de uma pessoa pra acumular as outras.
    ...
    private static string BuildImportSystemPrompt() => """
        ...
        Regras:
        1. Devolva SOMENTE o JSON pedido pelo schema, sem nenhum texto antes ou depois.
        2. Para cada bloco sugerido, primeiro verifique se alguma aba existente já cobre o assunto por categoria — compare o tema do bloco com o NOME de cada aba existente. Só deixe "targetTabId" vazio e preencha "suggestedNewTabName" quando NENHUMA aba existente corresponder claramente à categoria do conteúdo. Nunca preencha os dois ao mesmo tempo. EXCEÇÃO COM PRIORIDADE MÁXIMA: se o texto colado indicar explicitamente em qual aba guardar algo (ex.: "coloca isso na aba Teorias"), respeite o pedido — use o "targetTabId" da aba existente cujo nome corresponder (ignore maiúsculas/acentos); só se nenhuma existir com esse nome, preencha "suggestedNewTabName" exatamente com o nome pedido. Para blocos novos, NUNCA deixe "targetTabId" e "suggestedNewTabName" vazios ao mesmo tempo: se nenhuma aba servir, invente um nome curto e descritivo em "suggestedNewTabName".
        ...
        8. Antes de criar um bloco novo, confira a lista de blocos já existentes deste personagem. Se a ficha trouxer informação sobre o mesmo assunto/categoria de um bloco que já existe, prefira ATUALIZAR aquele bloco em vez de duplicar: preencha "targetBlockId" com o id dele e, em "content" (ou "payloadJson"), coloque APENAS a informação NOVA a acrescentar — NÃO repita o conteúdo que já está no bloco (o aplicativo anexa automaticamente ao existente, então reescrever o antigo só duplicaria). Para Card/Table, envie só as linhas novas; só reenvie uma linha existente se o valor mudou de fato.
    ...
    private static Dictionary<string, JsonElement> BuildSchema()
    {
        var itemSchema = new Dictionary<string, object>
        {
            ...
            ["properties"] = new Dictionary<string, object>
            {
                ...
                {
                    ["type"] = "string",
                    ["description"] = "Guid (as string) of an existing block to APPEND new information to. When set, put ONLY the new content in 'content'/'payloadJson' (never repeat the block's existing content — the app appends it). Empty string to create a new block instead.",
                },
                ...
                {
                    ["type"] = "string",
                    ["description"] = "Short name for a new tab. MUST be non-empty when both targetBlockId and targetTabId are empty (a new block always needs a destination); empty string when either of them is set.",
                },
                ...
                {
                    ["type"] = "string",
                    ["description"] = "Block text. When updating an existing block (targetBlockId set), include ONLY the new information to append — the app appends it to the existing content, so never repeat what's already there.",
                },
                ["payloadJson"] = new Dictionary<string, object>
                {
                    ["type"] = "string",
                    ["description"] = "JSON string payload for Card/Table blocks, or empty string otherwise. When updating an existing block, include ONLY the new rows (or a single row whose value actually changed) — not the rows already present.",
                },
            },
