...
- `StructureNoteRequest.NoteText` is capped at 4000 chars and `AnthropicSettings.MaxOutputTokens` (default 4096; was 1024, which truncated legitimate large notes) caps the response — both bound cost per call. A `max_tokens`-truncated response is a deterministic failure (structured-output JSON never parses when cut), so the gateway throws `AiResponseTooLargeException` → 400 with a "split your note" message instead of retrying or returning the misleading 503.
